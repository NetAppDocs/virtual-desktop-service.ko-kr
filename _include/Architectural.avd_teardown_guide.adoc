...
이 문서에서는 AVD 최종 사용자 액세스를 유지하면서 VDS 및 NetApp 제어 제거를 다룹니다. 앞으로는 기본 Azure/Windows 관리 툴을 사용할 것입니다. 이 프로세스가 완료되면 VDSsupport@netapp.com 으로 연락하여 NetApp이 백 엔드 및 청구 시스템을 정리할 수 있도록 하는 것이 좋습니다.
